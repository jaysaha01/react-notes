...
export default function Home() {
  ...
  import {redireact} from 'next/navigation';
...
// (put images into publick folder)
...
const Imageoptimization = () => {
  return (
    <div>
      //in this Image feature we get image by reposive size . but in normal img tag we have to put effect on optimization of the image
      <Image src={img} alt="next js logo" height={300} width={500}/>
    </div>
  )
}

export default Imageoptimization index
...
const Imageoptimization = () => {
  return (
    <div>
      <Image src={"url of the image"} alt="External image" height={300} width={500}/>
    </div>
  )
}
...
// if we not use font optimization then generate too many netwrok requests and see public to the font name
import React from 'react'
import {Poopins} from 'next/font/google'
...
const font = () => {
  return (
    <div>
      <h2 className={poopins.className}>font</h2>
    </div>
  )
}
...
// Static Assets (publick ka andar ka file)

//build time pe publick ka file change nahi hota he par src k andar ka file change hota he .
...
//npm run dev ➡ development build then show .next folder. Development build me code minify nahi hota taki ap development kar sako and debug kar sako

//npm run build ➡ Production build me code minify hojata he. now you can deploy
...
{
  process.env.NODE_ENV == "Development" ? <h1>Development Mode</h1> ? <h3>Production Mode</h3>
}
...
----------------------------------------
in production build console.log not working
*/

// custom environment variable ----------------

crate .env.local file => GOOGLEMAPAPIKEY = "my api key"
